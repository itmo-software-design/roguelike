...
![compositionDiagram.png](images/componentDiagram.png)

### Основные компоненты системы

* Game (Игровой менеджер): Центральный компонент, управляющий состоянием игры, обработкой пользовательского ввода и
  основным игровым циклом.
* Player (Игрок): Класс, представляющий персонажа игрока, его характеристики, инвентарь и действия.
* Level (Уровень): Отвечает за генерацию и управление текущим уровнем, включая карту, врагов, предметы и взаимодействие
  с ними.
* Enemy (Враг): Классы, представляющие противников на уровне, их поведение и взаимодействие с игроком.
* Inventory (Инвентарь): Система управления предметами игрока, включая экипировку и использование предметов.
* Item (Предмет): Абстрактный класс для всех типов предметов в игре, таких как оружие, броня и потребляемые предметы.
* UIManager (Менеджер пользовательского интерфейса): Отвечает за отображение информации игроку, включая основной экран
  игры, инвентарь, статусную строку и сообщения.
* InputHandler (Обработчик ввода): Обрабатывает пользовательский ввод с клавиатуры и передает команды в игровой
  менеджер.
* SaveLoadSystem (Система сохранения и загрузки): Управляет сохранением текущего состояния игры в файл и загрузкой
  сохраненных игр.
* CombatSystem (Система боя): Обрабатывает боевые взаимодействия между игроком и врагами, включая расчет урона и
  обновление состояния персонажей.
* Utilities (Утилиты): Вспомогательные классы и функции, такие как генератор случайных чисел для создания seed и другие
  вспомогательные методы.

### Взаимодействие компонентов

* Game использует InputHandler для обработки ввода от игрока и взаимодействует с UIManager для отображения информации.
* Game управляет состоянием Player и Level, обновляя их в соответствии с действиями игрока и игровым циклом.
...
2. **Player** (Игрок)
    ...
    * Свойства:
        ...
        * `name`: Имя игрока.
        * `health`, `attack`, `defense`: Характеристики персонажа.
        * `inventory`: Инвентарь игрока.
        * `position`: Текущее положение на карте.

    * Методы:

        * `move(direction: Direction)`: Перемещает игрока.
        * `attack(target: Character)`: Атакует цель.
        * `interact()`: Взаимодействует с объектами окружения.
        * `openInventory()`: Открывает инвентарь.
        * `updateStats()`: Обновляет характеристики (при экипировке и использовании предметов).
...
3. **Level** (Уровень)
    ...
    * Методы:
        ...
        * `checkForInteractableElements(position: Position)`: Проверяет наличие объектов для взаимодействия.
...
4. **Enemy** (Враг)
    ...
    * Свойства и методы аналогичны `Player`, но с уникальным поведением.
...
5. **Inventory** (Инвентарь)
    ...
    * Методы:
        * `open()`, `close()`: Открывает и закрывает инвентарь.
        * `addItem(item: Item)`, `removeItem(item: Item)`: Добавляет и удаляет предметы.
        * `selectItem(index: int)`: Выбирает предмет.
        * `equipItem(item: Item)`: Экипирует предмет.
        * `useItem(item: Item)`: Использует предмет.
...
* Перемещение и взаимодействие:
    ...
    * `Game` проверяет действия через методы Level (`validateMovement()`, `checkForInteractableElements()`).
...
* Инвентарь:
    * `Player` открывает `Inventory` через `openInventory()`.
    * Игрок взаимодействует с инвентарем, используя методы `Inventory` (`selectItem()`, `equipItem()`, `useItem()`).
    * `Player` обновляет свои характеристики через `updateStats()`.
...
* Поток ввода:
    * Пользовательский ввод обрабатывается `InputHandler`, который преобразует ввод в команды для Game.
...
* Сохранение состояния:
    * Все необходимые данные собираются из `Game`, `Player` и `Level` и сохраняются через `SaveLoadSystem`.
...
![img.png](images/sequenceDiagram.png)
...
![img.png](images/automataDiagram.png)
